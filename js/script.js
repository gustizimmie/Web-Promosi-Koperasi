...
document.querySelectorAll('.tab-btn').forEach(button => {
    button.addEventListener('click', () => {
        ...
        container.querySelectorAll('.pricing-grid').forEach(grid => {
            ...
            if (grid.id === tabTarget) {
                grid.classList.add('active');
            }
        });

        // Show/Hide CMS Access Box
        const cmsAccess = document.getElementById('cms-access-box');
        if (cmsAccess) {
            cmsAccess.style.display = (tabTarget === 'tab-online') ? 'block' : 'none';
        }
    });
});
